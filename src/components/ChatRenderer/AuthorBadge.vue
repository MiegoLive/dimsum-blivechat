...
<yt-live-chat-author-badge-renderer :type="authorTypeText">
  <el-tooltip :content="readableAuthorTypeText" placement="top">
    <div id="image" class="style-scope yt-live-chat-author-badge-renderer">
      ...
      <img v-if="privilegeType && privilegeType > 0" :src="`./static/img/icons/guard-level-${privilegeType}.png`"
...
import { getShowGuardLevelText } from './constants'
...
const authorTypeText = computed(() => {
  // 优先判断舰长
  if (props.privilegeType && props.privilegeType > 0) {
    return 'member'
  }

  return props.isAdmin ? 'moderator' : ''
})
...
const readableAuthorTypeText = computed(() => {
  ...
  return getShowGuardLevelText(props.privilegeType ?? 0)
})
</script>
