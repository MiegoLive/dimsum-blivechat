...
export function getImportStyle(allFontsStrs: string[]) {
  let allFonts = new Set<string>()
  ...
  let fontsNeedToImport = new Set<string>()
...
export function getAvatarStyle(config: { showAvatars: any; avatarSize: number }) {
  return `/* Avatars */
...
export function getTimeStyle(config: { showTime: any; timeColor: any; timeFont: any; timeFontSize: any; timeLineHeight: any }) {
  return `/* Timestamps */
...
export function getAnimationStyle(config: { animateIn: any; animateOut: any; fadeInTime: number; animateOutWaitTime: number; fadeOutTime: number; slide: any; reverseSlide: any }) {
...
export function fontsStrToArr(fontsStr: string) {
  return fontsStr ? fontsStr.split(',') : []
}

export function fontsArrToStr(fontsArr: any[]) {
  return fontsArr.join(',')
}

export function fontsStrToCss(fontsStr: any) {
...
function cssEscapeChar(char: any) {
...
function needEscapeChar(char: string) {
  let code = char.codePointAt(0)
  if (code !== undefined && 0x20 <= code && code <= 0x7E) {
    return char === '"' || char === '\\'
  }
  return true
}